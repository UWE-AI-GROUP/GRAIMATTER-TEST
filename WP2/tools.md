...
* Description: Provides a range of privacy attacks, including inference attacks and [model extraction](https://github.com/Trusted-AI/adversarial-robustness-toolbox/blob/main/notebooks/model-stealing-demo.ipynb). "ART supports all popular machine learning frameworks (TensorFlow, Keras, PyTorch, MXNet, scikit-learn, XGBoost, LightGBM, CatBoost, GPy, etc.), all data types (images, tables, audio, video, etc.) and machine learning tasks (classification, object detection, speech recognition, generation, certification, etc.)." Defences are mostly focused on adversarial examples (where inputs are crafted to cause the network to misclassify), however it also includes defences against model stealing (e.g., using deceptive perturbations), and defences against [generative adversarial network](https://en.wikipedia.org/wiki/Generative_adversarial_network) attacks where the latent representation of an input image is inferred and the original input reconstructed (e.g., by improving classifier robustness to perturbation). Provides three types of inference attacks.
  ...
  - [attribute inference](https://github.com/Trusted-AI/adversarial-robustness-toolbox/blob/main/notebooks/attack_attribute_inference.ipynb) aims at inferring the actual feature values of a record known to exist in the training data only by accessing the trained model and knowing few of the other features of the record. For example, a ML model trained on demographic data attacked with attribute inference could leak information about a person's exact age or salary. The attacked feature must be categorical, and with a relatively small number of possible values. The [API](https://adversarial-robustness-toolbox.readthedocs.io/en/latest/modules/attacks/inference/attribute_inference.html) supports a small number of black-box attacks and decision tree white-box attacks:
    + `AttributeInferenceBaseline` trains a classifier (default is a simple neural network with 100 hidden neurons) to learn the attacked feature from the rest of the features.
    + `AttributeInferenceBlackBox` trains a classifier (default is a simple neural network with 100 hidden neurons) to learn the attacked feature from the rest of the features and the target model's predictions for the samples under attack.
    + `AttributeInferenceMembership` searches for the target feature value that causes a (pre-trained) membership inference attack model to classify the sample as a member with the highest confidence.
    + `AttributeInferenceWhiteBoxLifestyleDecisionTree` is a white-box attack based on [Fredrikson et al. (2015)](https://doi.org/10.1145/2810103.2813677)
    + `AttributeInferenceWhiteBoxDecisionTree` is a white-box attack based on [Fredrikson et al. (2015)](https://doi.org/10.1145/2810103.2813677)
