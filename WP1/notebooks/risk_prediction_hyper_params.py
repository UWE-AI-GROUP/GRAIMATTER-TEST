...
import matplotlib as plt #pylab as plt 
import pandas as pd
...
parser = argparse.ArgumentParser(description="Predicts and plots attack metrics based on target metrics.")
parser.add_argument(action='store', dest='results_csv_file', help='Csv file that contains results from hyper-parameter search loop. It is assumed the file is located in "experiments" directory, so please provide path and filename, e.g. RF/Random_Forest_loop_results.csv')
parser.add_argument(action='store', dest='config_filename', help='Configuration file in json format used to generate the results')
parser.add_argument('--include-images', action='store_true', dest='img', help='Optional argument which forces images datasets to be included in the analysis. Default: remove them from results.')

args = parser.parse_args()
config_filename = args.config_filename
results_csv_file = args.results_csv_file
include_img = args. img
...
#load hyper-params json file
def load_hyper_params_json(config_file):
    logger.info("Loading hyper-parameters from file: %s", config_file)
    with open(config_file, 'r') as f:
        config = json.loads(f.read())

    experiment_params = config['experiment_params']
    exp_pars = {}
    for classifier, params in experiment_params.items():
       for k,v in params.items(): 
           exp_pars[k] = v
    results_filename = config['results_filename']
    return(exp_pars, results_filename)
...
experiment_params, results_filename = load_hyper_params_json(config_file)

print('experiment_params', experiment_params)
print('res file', results_filename)
