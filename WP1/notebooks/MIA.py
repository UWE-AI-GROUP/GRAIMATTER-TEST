...
def create_mia_data(clf, xtrain, xtest): 
    """
    This function predict the probability of train and test data (from building the model split) belonging to their corresponding train or test respectively and it also creates labels.
...
def run_membership_inference_attack(clf_name, model, xtrain, xtest, MIA_classifier, MIA_name):
    """
    This function performs MIA (membership inference attack) and plots metrics.
    ...
    plotROC_classifier(mi_rf, mi_test_x, mi_test_y, name+" - MIA "+MIA_name)
